...
// ------------------------------------------------------------
const WORLD_W = 2400; // total world width in pixels
const WORLD_H = 1350; // total world height in pixels
...
let player = {
  ...
  direction: { x: 0, y: 1 }, // for shooting
  facing: "down",            // for sprite row
...
function preload() {
  ...
  characterImg = loadImage("assets/images/charactersprite.png");
  enemyImg = loadImage("assets/images/enemysprite.png");
  blockImg = loadImage("assets/images/block.png");

  // Load sounds
  backgroundMusic = loadSound("assets/sounds/background-music.m4a");
  hitSound = loadSound("assets/sounds/hit-sound.mp3");
  jumpSound = loadSound("assets/sounds/jump-sound.mp3");
  winSound = loadSound("assets/sounds/win-sound.mp3");

  // load people
  enemySheet = loadImage("assets/images/enemysprite.png");
  characterSheet = loadImage("assets/images/charactersprite.png");
}
...
// ------------------------------------------------------------
// drawBackground()
// Draws the background image tiled across the world.
// ------------------------------------------------------------
function drawBackground() {
  // Tile the background image across the entire world
  if (backgroundImg) {
    imageMode(CORNER);
    let tileW = backgroundImg.width;
    let tileH = backgroundImg.height;
...
// ------------------------------------------------------------
// drawBoss()
// Drawn in world coordinates using sprite sheet.
// Displays as a larger enemy sprite with animation.
// ------------------------------------------------------------
function drawBoss() {
  if (!boss || !enemyImg) return;

  // Enemy sprite sheet: 4 columns x 4 rows
  let cols = 4;
  let rows = 4;
  let frameW = enemyImg.width / cols;
  let frameH = enemyImg.height / rows;

  // Use row 0 (down) for boss, animate through frames
  let col = floor((boss.blobT * 0.5) % 4);
  let row = 0;
  ...
  image(
    ...
    boss.x,
    boss.y,
    120,  // displayed width on screen (larger for boss)
    140,  // displayed height on screen (larger for boss)
    sx,
    sy,
    frameW,
    frameH
...
// drawEnemies()
// Drawn in world coordinates using sprite sheet.
// Each enemy animates based on direction towards player.
// ------------------------------------------------------------
function drawEnemies() {
  ...
  for (let i = 0; i < enemies.length; i++) {
    ...
    // Determine direction towards player for sprite row
    let dx = player.x - e.x;
    let dy = player.y - e.y;
    let facing = "down";
    ...
    // Animate through walking frames
    let col = floor(frameCount / 8) % 4;
    let row = rowMap[facing];
    ...
    image(
      ...
      e.x,
      e.y,
      60,   // displayed width on screen
      70,   // displayed height on screen
      sx,
      sy,
      frameW,
      frameH
...
// ------------------------------------------------------------
// drawPlayer()
// Drawn in world coordinates. Uses character sprite image.
...
function drawPlayer() {
  ...
  // Your sheet is 4 columns x 4 rows
  let cols = 4;
  let rowsCount = 4;

  // Automatically calculate the real frame size
  let frameW = characterImg.width / cols;
  let frameH = characterImg.height / rowsCount;
  ...
  let col = player.currentFrame;
  let row = rowMap[player.facing];
  ...
  image(
    ...
    player.x,
    player.y,
    50,   // displayed width on screen
    65,   // displayed height on screen
    sx,
    sy,
    frameW,
    frameH
